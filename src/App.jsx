...
// Googleアラート等のRSSをここに設定可能（現在はBBC World News）
    ...
    <div className="w-full h-full bg-slate-950">
      <ComposableMap projectionConfig={{ scale: 220 }} className="w-full h-full">
        <ZoomableGroup 
          ...
          minZoom={1} 
          maxZoom={8} 
          /* * 解決案：左右のドラッグ範囲を広げ(-500〜1300)、世界の両端を中央に寄せられるように。
           * 上下のドラッグ範囲は地図の描画エリア(0〜600)に厳密に制限し、黒い空間が見えないように調整。
           */
...
const GlobalAnalytics = ({ data, isExpanded }) => {
  ...
  return (
    <div className={`grid gap-6 h-full transition-all ${isExpanded ? 'lg:grid-cols-12' : 'lg:grid-cols-2'}`}>
      ...
      {isExpanded && (
        <div className="lg:col-span-4 bg-slate-900/80 backdrop-blur-xl border border-primary/30 flex flex-col overflow-hidden rounded-lg shadow-[0_0_30px_rgba(6,182,212,0.1)] animate-in fade-in slide-in-from-right duration-500">
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/90 backdrop-blur-2xl border-l border-primary/30 shadow-2xl">
      <div className="p-5 border-b border-white/10 flex justify-between items-start">
        <div>
          <div className="text-[10px] text-primary animate-pulse tracking-widest mb-1 font-mono uppercase">Target Acquired</div>
          <h2 className="text-2xl font-bold text-white tracking-tight">{master.name}</h2>
          <div className="text-xs font-mono text-slate-500 mt-1 uppercase">{master.iso3} | {canonical?.politics?.regime_type || 'N/A'}</div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 bg-slate-800/60 border border-white/5 rounded-md"><div className="text-[9px] text-slate-500 mb-1 flex items-center gap-1"><Users size={10}/>POPULATION</div><div className="font-mono text-emerald-400 text-lg">{canonical?.society?.population?.value?.toLocaleString() || '0'}</div></div>
          <div className="p-3 bg-slate-800/60 border border-white/5 rounded-md"><div className="text-[9px] text-slate-500 mb-1 flex items-center gap-1"><Activity size={10}/>GDP (NOM)</div><div className="font-mono text-blue-400 text-lg">${((canonical?.economy?.gdp_nominal?.value || 0) / 1e9).toFixed(1)}B</div></div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 pt-2">{ui_view?.tags?.map(t => <span key={t} className="px-2 py-0.5 rounded border border-white/10 text-[9px] text-slate-400 uppercase font-mono bg-white/5">{t}</span>)}</div>
      </div>
    </div>
  );
};
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-950 relative font-sans text-slate-200">
      ...
      <header className="absolute top-0 left-0 right-0 h-14 flex items-center px-6 justify-between z-[80] bg-gradient-to-b from-slate-950/80 to-transparent pointer-events-none">
        <div className="flex items-center gap-3 pointer-events-auto">
          ...
          <h1 className="text-lg font-bold tracking-widest font-mono text-glow uppercase">WorldDash <span className="text-[10px] text-slate-500 font-normal tracking-normal">System_Active</span></h1>
        </div>
        <button onClick={toggleFs} className="pointer-events-auto text-slate-400 hover:text-primary transition-all flex items-center gap-2 border border-white/10 px-3 py-1.5 rounded bg-slate-900/60 backdrop-blur-md text-[10px] font-bold shadow-lg uppercase tracking-wider">
      ...
      <main className="flex-1 relative">
        ...
        {hoverInfo && (
          <div className="fixed z-[120] px-3 py-2 text-[10px] bg-slate-900/95 backdrop-blur-xl border border-primary/50 text-slate-100 font-mono pointer-events-none shadow-2xl" style={{ left: hoverInfo.x + 15, top: hoverInfo.y + 15 }}>
            ...
            <div className="opacity-70 font-mono uppercase tracking-widest">ID_REF: {hoverInfo.iso3}</div>
          </div>
        )}

        <aside className={`absolute top-0 bottom-0 right-0 w-80 md:w-96 transform transition-all duration-700 z-[90] ${selectedIso ? 'translate-x-0' : 'translate-x-full'} shadow-[-20px_0_30px_rgba(0,0,0,0.5)]`}>
        ...
        <footer className={`absolute bottom-0 left-0 right-0 z-[100] bg-slate-950/95 backdrop-blur-3xl border-t border-primary/30 transition-all duration-700 flex flex-col ${isAnalyticsOpen ? 'h-[calc(100vh-3.5rem)]' : 'h-10'} shadow-[0_-20px_50px_rgba(0,0,0,0.8)]`}>
          <button onClick={() => setIsAnalyticsOpen(!isAnalyticsOpen)} className="h-10 w-full flex items-center justify-center gap-3 text-[10px] font-bold tracking-[0.3em] text-primary/70 hover:text-primary hover:bg-primary/5 transition-all shrink-0 border-b border-white/5 pointer-events-auto group">
